...
export default function AnnouncementsTicker() {
  ...
  if (error) {
    ...
    // エラー時は何も表示しないか、エラーメッセージを表示
    return (
        <div className="bg-red-100 border-l-4 border-red-500 text-red-700 p-3 text-sm flex items-center space-x-2 h-10 overflow-hidden">
  ...
  if (!announcements) {
    // ローディング表示（任意）
    return (
      <div className="bg-blue-50 border-l-4 border-blue-400 text-blue-700 p-3 text-sm flex items-center space-x-2 h-10 overflow-hidden">
  ...
  if (announcements.length === 0) {
    // お知らせがない場合の表示
    return (
      <div className="bg-gray-100 border-l-4 border-gray-400 text-gray-600 p-3 text-sm flex items-center space-x-2 h-10 overflow-hidden">
  ...
  // currentAnnouncement が存在しない場合のフォールバック (念のため)
  ...
  return (
    // 背景色などを変更して NotificationTicker と区別
    <div className="bg-yellow-50 border-l-4 border-yellow-400 text-yellow-800 p-3 text-sm flex items-center space-x-2 h-10 overflow-hidden">
